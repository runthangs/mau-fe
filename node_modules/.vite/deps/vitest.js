...
async function saveInlineSnapshots(environment, snapshots) {
  const MagicString = (await import("./magic-string.es-BFQ56Z4P.js")).default;
...
function createExpectPoll(expect2) {
  return function poll(fn2, options = {}) {
    var _a;
    const defaults = ((_a = getWorkerState().config.expect) == null ? void 0 : _a.poll) ?? {};
...
function recordAsyncExpect2(_test2, promise, assertion, error) {
  ...
  if (test5 && promise instanceof Promise) {
    ...
    test5.onFinished.push(() => {
      var _a;
      if (!resolved) {
        const stack = (((_a = globalThis.__vitest_worker__) == null ? void 0 : _a.onFilterStackTrace) || ((s) => s || ""))(error.stack);
        console.warn([
          `Promise returned by \`${assertion}\` was not awaited. `,
          "Vitest currently auto-awaits hanging assertions at the end of the test, but this will cause the test to fail in Vitest 3. ",
          "Please remember to await the assertion.\n",
...
var SnapshotPlugin = (chai, utils) => {
  ...
  utils.addMethod(chai.Assertion.prototype, "toMatchInlineSnapshot", function __INLINE_SNAPSHOT__(properties, inlineSnapshot, message) {
    ...
    if (test5.each || ((_a = test5.suite) == null ? void 0 : _a.each)) throw new Error("InlineSnapshot cannot be used inside of test.each or describe.each");
    const expected = utils.flag(this, "object");
...
function createVitest() {
  ...
  const utils = {
    useFakeTimers(config2) {
      var _a, _b, _c, _d;
      if (isChildProcess()) {
        if (((_a = config2 == null ? void 0 : config2.toFake) == null ? void 0 : _a.includes("nextTick")) || ((_d = (_c = (_b = state().config) == null ? void 0 : _b.fakeTimers) == null ? void 0 : _c.toFake) == null ? void 0 : _d.includes("nextTick"))) throw new Error('vi.useFakeTimers({ toFake: ["nextTick"] }) is not supported in node:child_process. Use --pool=threads if mocking nextTick is required.');
      }
    ...
    mockObject(value, options) {
      return _mocker().mockObject({ value }, void 0, (options == null ? void 0 : options.spy) ? "autospy" : "automock").value;
    },
...
function getImporter(name) {
  ...
  return ((_a = parseSingleStack(stackArray[stackArray.findLastIndex((stack) => {
    return stack.includes(` at Object.${name}`) || stack.includes(`${name}@`) || stack.includes(` at ${name} (`);
  }) + 1])) == null ? void 0 : _a.file) || "";
}
...
var resolve3 = function(...arguments_) {
  ...
  for (let index3 = arguments_.length - 1; index3 >= -1 && !resolvedAbsolute; index3--) {
    const path = index3 >= 0 ? arguments_[index3] : cwd3();
  ...
  for (let index3 = 0; index3 <= path.length; ++index3) {
    ...
    if (char === "/") {
      if (!(lastSlash === index3 - 1 || dots === 1)) if (dots === 2) {
        ...
        res.length > 0 ? res += `/${path.slice(lastSlash + 1, index3)}` : res = path.slice(lastSlash + 1, index3), lastSegmentLength = index3 - lastSlash - 1;
...
var DecodedMap2 = class {
  constructor(map2, from) {
    ...
    this.map = map2;
    const { mappings, names, sources } = map2;
    this.version = map2.version, this.names = names || [], this._encoded = mappings || "", this._decodedMemo = memoizedState2(), this.url = from, this.resolvedSources = (sources || []).map(
...
var EvaluatedModules = class {
  ...
  getModuleSourceMapById(id) {
    ...
    const mapString = (_a = MODULE_RUNNER_SOURCEMAPPING_REGEXP.exec(
      mod.meta.code
    )) == null ? void 0 : _a[1];
